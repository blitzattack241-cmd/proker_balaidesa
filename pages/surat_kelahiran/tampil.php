...
if (mysqli_connect_errno()) {
    ...
    $query = false;
    // Cek kandidat nama tabel untuk surat kelahiran
    $tableCandidates = ['tb_surat_kelahiran', 'surat_kelahiran'];
    foreach ($tableCandidates as $tableCandidate) {
        $check = mysqli_query($koneksi, "SHOW TABLES LIKE '$tableCandidate'");
        if ($check && mysqli_num_rows($check) > 0) {
            // Ambil data surat kelahiran diurutkan berdasarkan surat terbaru
            $sql = "SELECT id_surat, nomor_surat, nama_kepala_keluarga, nomor_kk, nama_bayi, tanggal_surat 
                    FROM `$tableCandidate` 
...
<div class="container-fluid px-4 py-3">
    ...
    <div class="d-flex justify-content-between align-items-center mb-2">
        <div>
            <h3 class="page-title-modern mt-2 mb-1">Surat Kelahiran</h3>
            <ol class="breadcrumb breadcrumb-modern mb-0">
                <li class="breadcrumb-item"><a href="index.php?page=dashboard"
                        class="text-decoration-none">Dashboard</a></li>
                <li class="breadcrumb-item active">Daftar Surat Kelahiran</li>
            </ol>
        </div>
    </div>
    ...
    <div class="card card-modern mb-4">
        ...
        <div class="card-header-modern d-flex justify-content-between align-items-center">
            <div class="card-header-title d-flex align-items-center">
                <i class="fas fa-baby me-2 text-primary"></i> Data Surat Keterangan Kelahiran Berugenjang
            </div>
            <!-- Tombol Tambah Halaman Baru Modern -->
            <a href="index.php?page=surat-kelahiran-tambah" class="btn btn-primary btn-tambah-modern">
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="fw-bold text-primary text-uppercase">
                                    <?= htmlspecialchars($data['nama_bayi']); ?></div>
                            </td>
                            <td>
                                <div class="fw-semibold text-dark">
                                    <?= htmlspecialchars($data['nama_kepala_keluarga']); ?></div>
                            </td>
                            <td>
                                <span class="badge bg-light text-dark p-2 border font-monospace text-start w-100">
                                    ...
                                    <?= htmlspecialchars($data['nomor_kk']); ?>
                                </span>
                            </td>
                            <td class="text-center">
                                <div class="d-flex justify-content-center align-items-center gap-2">
                                    ...
                                    <!-- Tombol Cetak -->
                                    <a href="pages/surat_kelahiran/cetak.php?id=<?= $data['id_surat']; ?>"
                                        target="_blank"
                                    ...
                                    <!-- Tombol Update -->
                                    <a href="index.php?page=surat-kelahiran-edit&id=<?= $data['id_surat']; ?>"
                                    ...
                                    <!-- Tombol Delete -->
                                    <a href="pages/surat_kelahiran/proses_hapus.php?id=<?= $data['id_surat']; ?>"
                                        class="btn btn-sm d-flex flex-column align-items-center justify-content-center action-btn btn-delete-modern"
                                        onclick="return confirm('Apakah Anda yakin ingin menghapus data surat kelahiran ini?');"
                                        title="Hapus">
