...
if (mysqli_connect_errno()) {
    ...
    $query = false;
    // Pengecekan nama tabel fleksibel (tb_sktm_rawat atau variasi lainnya)
    $tableCandidates = ['tb_sktm_rawat', 'sktm_rawat', 'tb_sktm_pasien', 'sktm_pasien'];
    foreach ($tableCandidates as $tableCandidate) {
        $check = mysqli_query($koneksi, "SHOW TABLES LIKE '$tableCandidate'");
        if ($check && mysqli_num_rows($check) > 0) {
            $query = mysqli_query($koneksi, "SELECT * FROM `$tableCandidate` ORDER BY `tanggal_surat` DESC");
            break;
        }
    }
}
...
<div class="container-fluid px-4 py-3">
    ...
    <div class="d-flex justify-content-between align-items-center mb-2">
        <div>
            <h3 class="page-title-modern mt-2 mb-1">SKTM Pembebasan Rawat</h3>
            <ol class="breadcrumb breadcrumb-modern mb-0">
                <li class="breadcrumb-item"><a href="index.php?page=dashboard"
                        class="text-decoration-none">Dashboard</a></li>
                <li class="breadcrumb-item active">Daftar Surat Pembebasan Rawat</li>
            </ol>
        </div>
    </div>
    ...
    <div class="card card-modern mb-4">
        ...
        <div class="card-header-modern d-flex justify-content-between align-items-center">
            <div class="card-header-title d-flex align-items-center">
                <i class="fas fa-file-medical me-2 text-primary"></i> Data SKTM Pembebasan Biaya Rawat
            </div>
            <!-- Tombol Tambah Halaman Baru Modern -->
            <a href="index.php?page=sktm-rawat-tambah" class="btn btn-primary btn-tambah-modern">
                ...
                <table class="table table-hover table-striped align-middle" id="dataTable" width="100%" cellspacing="0">
                    <thead class="table-dark">
                        <tr>
                            ...
                            <th width="20%">Rumah Sakit Tujuan</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><?= $tanggal; ?></td>
                            <td class="fw-bold text-primary"><?= htmlspecialchars($data['nama_pemohon']); ?></td>
                            <td>
                                <span class="badge bg-light text-dark p-2 border">
                                    <i class="fas fa-hospital me-1 text-danger"></i>
                                    <?= htmlspecialchars($data['rumah_sakit_tujuan']); ?>
                                </span>
                            </td>
                            <td class="text-center">
                                <div class="d-flex justify-content-center align-items-center gap-2">
                                    ...
                                    <!-- Tombol Cetak -->
                                    <a href="pages/sktm_rawat/cetak.php?id=<?= $data['id_sktm']; ?>" target="_blank"
                                    ...
                                    <!-- Tombol Update -->
                                    <a href="index.php?page=sktm-rawat-edit&id=<?= $data['id_sktm']; ?>"
                                    ...
                                    <!-- Tombol Delete -->
                                    <a href="pages/sktm_rawat/proses_hapus.php?id=<?= $data['id_sktm']; ?>"
                                        class="btn btn-sm d-flex flex-column align-items-center justify-content-center action-btn btn-delete-modern"
                                        onclick="return confirm('Apakah Anda yakin ingin menghapus data surat pembebasan rawat ini beserta data detail pasien di dalamnya?');"
                                        title="Hapus">
                        ...
                        <?php
                            ...
                            ?>
                        <tr>
                            <td colspan="6" class="text-center text-muted py-4">Belum ada data surat keterangan tidak
                                mampu (rawat).</td>
                        </tr>
                        <?php } ?>
                    </tbody>
