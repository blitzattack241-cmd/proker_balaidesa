...
// 1. Query data utama SKTM Rawat dan Pejabat
$query = mysqli_query($koneksi, "
    SELECT s.*, p.nama_pejabat, p.jabatan 
    FROM tb_sktm_rawat s
...
$qr_token = dapatkanTokenVerifikasi($koneksi, 'sktm_rawat', $id_sktm, $data['nomor_surat'] ?? '');

// 2. Query detail data pasien (Relasi One-to-Many)
$query_pasien = mysqli_query($koneksi, "
    SELECT * FROM tb_sktm_rawat_pasien 
...
<body>
    ...
    <div class="preview-header">
        <div class="preview-title">Sistem Balai Desa - Pratinjau Cetak SKTM Rawat</div>
    ...
    <div class="kop-teks">
            ...
            <table class="tabel-data">
                <tr>
                    ...
                    <td style="font-weight: bold; text-transform: uppercase;">
                        <?= htmlspecialchars($data['nama_pemohon']); ?>
                    </td>
                </tr>
                <tr>
                    <td>2.</td>
                ...
                <!-- POIN 10: KEPERLUAN & DETAIL PASIEN -->
                <tr>
                    <td>10.</td>
                    <td>Keperluan</td>
                    <td>:</td>
                    <td style="text-align: justify;">
                        Permohonan Pembebasan Biaya Rawat Inap dan Rawat Jalan di
                        <strong><?= htmlspecialchars($data['rumah_sakit_tujuan']); ?></strong>, atas nama :

                        <!-- Tabel List Pasien Dinamis -->
                        <table class="tabel-pasien">
                            ...
                            if (mysqli_num_rows($query_pasien) > 0) {
                                while ($pasien = mysqli_fetch_assoc($query_pasien)) {
                                    ?>
                                    <tr>
                                        <td class="col-p-no"><?= $no_p++; ?>.</td>
                                        <td class="col-p-nama" style="text-transform: uppercase;">
                                            <?= htmlspecialchars($pasien['nama_pasien']); ?>
                                        </td>
                                        <td class="col-p-titik">:</td>
                                        <td class="col-p-nik"><?= htmlspecialchars($pasien['nik_pasien']); ?></td>
                                    </tr>
                                    <?php
                                }
                            } else {
                                // Fallback jika relasi tabel rawat_pasien kosong
                                ?>
                                <tr>
                                    <td class="col-p-no">1.</td>
                                    <td class="col-p-nama" style="text-transform: uppercase;">
                                        <?= htmlspecialchars($data['nama_pemohon']); ?>
                                    </td>
                                    <td class="col-p-titik">:</td>
                                    <td class="col-p-nik"><?= htmlspecialchars($data['no_ktp']); ?></td>
                                </tr>
                            <?php } ?>
                        </table>
                    </td>
            ...
            <div class="container-ttd">
                <div class="row-ttd-atas">
                    <div class="col-ttd">
                        ...
                        <strong><u><?= htmlspecialchars($data['nama_pemohon']); ?></u></strong>
                    </div>
                    <div class="col-ttd">
        ...
        <div class="page">
            <div class="lampiran-judul">
                Lampiran Dokumen Kelayakan SKTM Rawat<br>
                <span style="font-size: 10pt; text-transform: none; font-weight: normal;">Arsip Foto Kondisi Rumah - An.
                    <?= htmlspecialchars($data['nama_pemohon']); ?></span>
            </div>

            <div class="grid-foto">
                <div class="box-foto">
                    <?php if (!empty($data['foto_depan']) && file_exists("../../assets/img/sktm_rawat/" . $data['foto_depan'])): ?>
                        <img src="../../assets/img/sktm_rawat/<?= $data['foto_depan']; ?>" alt="Tampak Depan">
                    <?php else: ?>
                ...
                <div class="box-foto">
                    <?php if (!empty($data['foto_ruang_tamu']) && file_exists("../../assets/img/sktm_rawat/" . $data['foto_ruang_tamu'])): ?>
                        <img src="../../assets/img/sktm_rawat/<?= $data['foto_ruang_tamu']; ?>" alt="Ruang Tamu">
                    <?php else: ?>
                        <div style="height: 95px; line-height: 95px; background: #eee; color: #888; font-size: 9pt;">Tidak
                            Ada Foto</div>
                    <?php endif; ?>
                ...
                <div class="box-foto">
                    <?php if (!empty($data['foto_kamar']) && file_exists("../../assets/img/sktm_rawat/" . $data['foto_kamar'])): ?>
                        <img src="../../assets/img/sktm_rawat/<?= $data['foto_kamar']; ?>" alt="Kamar Tidur">
                    <?php else: ?>
                        <div style="height: 95px; line-height: 95px; background: #eee; color: #888; font-size: 9pt;">Tidak
                            Ada Foto</div>
                    <?php endif; ?>
                ...
                <div class="box-foto">
                    <?php if (!empty($data['foto_dapur']) && file_exists("../../assets/img/sktm_rawat/" . $data['foto_dapur'])): ?>
                        <img src="../../assets/img/sktm_rawat/<?= $data['foto_dapur']; ?>" alt="Dapur">
                    <?php else: ?>
                        <div style="height: 95px; line-height: 95px; background: #eee; color: #888; font-size: 9pt;">Tidak
                            Ada Foto</div>
                    <?php endif; ?>
                ...
                <div class="box-foto foto-full">
                    <?php if (!empty($data['foto_toilet']) && file_exists("../../assets/img/sktm_rawat/" . $data['foto_toilet'])): ?>
                        <img src="../../assets/img/sktm_rawat/<?= $data['foto_toilet']; ?>" alt="Kamar Mandi / Toilet">
                    <?php else: ?>
